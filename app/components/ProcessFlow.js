...
export default function ProcessFlow({ flow, fields }) {
  ...
  const containerVariants = !lowResourceMode ? {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      transition: {
  ...
  const itemVariants = !lowResourceMode ? {
  ...
  return (
    <div className="h-full bg-white/70 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-100/50 p-6 border border-gray-100 flex flex-col">
      ...
      <AnimatePresence>
        {showFields && (
          <motion.div
            initial={!lowResourceMode ? { height: 0, opacity: 0 } : false}
            animate={!lowResourceMode ? { height: 'auto', opacity: 1 } : false}
            exit={!lowResourceMode ? { height: 0, opacity: 0 } : false}
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <motion.div
          ...
          animate={!lowResourceMode ? "show" : false}
        >
          <AnimatePresence mode="popLayout">
            {flow.map((step, index) => (
              <motion.div
                ...
                initial={!lowResourceMode ? "hidden" : false}
                animate={!lowResourceMode ? "show" : false}
                exit={!lowResourceMode ? { opacity: 0, x: -100 } : false}
              ...
              >
                ...
                {index < flow.length - 1 && (
                  <motion.div 
                    ...
                    initial={!lowResourceMode ? { scale: 0 } : false}
                    animate={!lowResourceMode ? { scale: 1 } : false}
                  >
